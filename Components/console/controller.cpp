...
{
    int l = strlen(text) + 1;
    Driver::instance().sendPacket(console::Controller::MessageReceived, l, (uint8_t*)text);
}

bool console::Controller::sendOSC(uint8_t channel, uint8_t length, uint16_t index, const uint8_t* data)
{
    if (!Driver::instance().checkBeforeSend(length + 100)) return false;
    uint8_t buf[256];
    buf[0] = channel;
    buf[1] = (index >> 8) & 0xFF;
    buf[2] = index & 0xFF;
    ...
    Driver::instance().sendPacket(console::Controller::Oscilloscope, length+3, buf);
    return true;
}
...
void console::Controller::updateIntField(uint16_t command, int32_t value)
{
    ...
    Driver::instance().sendPacket(console::Controller::IntFieldReceived, 6, buf);
}
...
void console::Controller::updateTextField(uint16_t command, uint8_t length, uint8_t* data)
{
    ...
    Driver::instance().sendPacket(console::Controller::TextFieldReceived, length+2, buf);
}
